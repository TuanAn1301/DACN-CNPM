...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("test_search_order_by_status.log", encoding='utf-8'),
...
class TestSearchOrderByStatus:
    def __init__(self):
        ...
        # Test data - search by status: 0 = Chưa duyệt đơn
        self.test_data = {
            ...
            'username': 'admin',
            'password': 'admin',
            'search_status': '0',  # 0 = Chưa duyệt đơn
            'status_name': 'Chưa duyệt đơn',
            'order_count': 0
        }
    ...
    def search_by_status(self):
        ...
        try:
            self.add_test_step(
                ...
                f"Hiển thị danh sách đơn hàng có trạng thái: {self.test_data['status_name']}",
                "Đang tìm kiếm...",
                "PENDING"
            )
            ...
            select.select_by_value(self.test_data['search_status'])
            ...
            WebDriverWait(self.driver, 10).until(
                lambda d: self.test_data['search_status'] in d.current_url or
                        d.find_elements(By.CSS_SELECTOR, "table tbody tr")
            )
            
            screenshot_path = self.take_screenshot("search_by_status_result")
            ...
            try:
                ...
                if self.test_data['search_status'] in current_url or f"status={self.test_data['search_status']}" in current_url:
                    result_msg = f"Tìm kiếm thành công. Tìm thấy {self.test_data['order_count']} đơn hàng với trạng thái: {self.test_data['status_name']}"
                else:
                    result_msg = f"Đã chọn trạng thái {self.test_data['status_name']}. Tìm thấy {self.test_data['order_count']} đơn hàng"
                ...
                self.test_steps[-1].update({
                    'status': 'PASS',
                    'actual': f"Đã chọn trạng thái tìm kiếm: {self.test_data['status_name']}",
                    'screenshot': screenshot_path
                })
                return True
                
        except Exception as e:
            error_msg = f"Lỗi khi tìm kiếm theo trạng thái: {str(e)}"
            logger.error(error_msg, exc_info=True)
            screenshot_path = self.take_screenshot("search_by_status_error")
            self.test_steps[-1].update({
                'status': 'FAIL',
                'actual': error_msg,
                'screenshot': screenshot_path
    ...
    def create_excel_report(self):
        ...
        try:
            ...
            # Add title
            title = "KẾT QUẢ KIỂM THỬ TÌM KIẾM ĐƠN HÀNG THEO TRẠNG THÁI"
            ...
            ws['A4'] = f"Trạng thái tìm kiếm: {self.test_data.get('status_name', 'N/A')}"
            ...
            report_path = os.path.join(self.report_dir, f'test_search_order_by_status_{timestamp}.xlsx')
            wb.save(report_path)
    ...
    def run_test(self):
        ...
        try:
            ...
            try:
                ...
                print("\n" + "="*60)
                print("KẾT THÚC KIỂM THỬ TÌM KIẾM ĐƠN HÀNG THEO TRẠNG THÁI")
                print("="*60)
